...
# def every_week(X, a, delta_day,delta_predictor,predict_begin,predict_end):
    # X
...
def oneweek(X,divid,predict_begin,predict_end):
    # 根据前一周预测
    ...
    s=sum(X)/len(X)
    delta = float(predict_end - predict_begin)
    # delta=2 #4 - 25point 3-19 1-6 2-11 8-40 15-58 20-58  40-
    # divid = 2.5  # 起到加权的作用 2-72.79 2.5-75.477 3-73.38
    s = int(s *delta*divid // 1)
    if s < 0:
        s = 0
    return s
...
def double_exp_smoothing(X, a, delta_day,delta_predictor):##
    ...
    for j in range(1, delta_day + 1):
        uu += At + Bt * int(j+delta_predictor)
    if uu < 0:
        uu = 0
    uu = int(uu*delta // 1)
    # Xt.append(uu)
...
def triple_exp_smoothing(X,a,delta_day,total_days,delta_predictor):
    b=a
    c=a
    length_n = total_days
    S3_1 = []
    ...
    for j in range(delta_predictor+1, delta_predictor+delta_day + 1):
